...
const handleSubmit = async (e) => {
    ...
    setMessages((prev) => {
        ...
        if (prev.length > 0 && prev[prev.length - 1].role === "user" && prev[prev.length - 1].content === message) {
        return prev;
        }
        ...
        {messages.map((msg) => (
            <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-md p-4 rounded-xl ${msg.role === 'user' ? 'bg-white text-black' : 'bg-black text-white border border-white'}`}>
                    {msg.content.split('\n').map((line, idx) => {
                        const isCommand = /^(?:\$|git |npm |yarn |npx )/.test(line.trim());
                        return isCommand ? (
                            <SyntaxHighlighter
                            key={`command-${idx}`}
                                language="bash"
                                style={oneDark}
                                customStyle={{ padding: '8px', borderRadius: '6px', margin: '4px 0' }}
                            >
                                {line}
                            </SyntaxHighlighter>
                        ) : (
